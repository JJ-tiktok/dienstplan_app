...
import { APP_NAME_SHORT, ICAL_UID_DOMAIN } from '@/lib/branding';
import type { Match } from '@/types';
import {
  getMatchCalendarSummaryLine,
  getMatchPublicTitle,
 ...
 */
export const generateICalendar = (matches: Match[]): string => {
  ...
  matches.forEach(match => {
    ...
    const summary = getMatchPublicTitle(match);
    const description = `${getMatchCalendarSummaryLine(match)}\nOrt: ${match.location || 'Siehe Termin'}`;
    const location = match.location || 'Siehe Termin';

    ics += 'BEGIN:VEVENT\r\n';
    ics += `UID:match-${match.id}@${ICAL_UID_DOMAIN}\r\n`;
...
export const downloadICalendar = (
  matches: Match[],
  filename: string = 'dienstplan-termine.ics'
): void => {
