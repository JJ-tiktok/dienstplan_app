...
export default function MatchDetail() {
  ...
  const handleSignUp = async () => {
    ...
    if (!selectedSlot || normalizedName.length < 2) {
      setAlertDialog({
        isOpen: true,
    ...
    // Validate contact (email required)
    ...
    setSubmittingSlotId(selectedSlot.id);

    try {
      const result = await bookSlot(selectedSlot.id, normalizedName, normalizedEmail);
  ...
  const handleConfirmCancellation = async () => {
    ...
    try {
      ...
      setAlertDialog({
        isOpen: true,
        title: 'Fehler',
        message: 'Fehler beim Beantragen der Austragung. Bitte erneut versuchen.',
        variant: 'error',
      });
    } finally {
      setIsSubmitting(false);
  ...
  if (!match) return <div className="p-6">Termin nicht gefunden.</div>;
      ...
      <div className="pt-24 px-6 pb-10 flex flex-col items-center text-center">
        ...
        <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-none mb-2">
          {isSportMatch(match) ? (
            <>
              {match.team && (
                <span className="text-blue-600 text-xs font-bold bg-blue-50 px-3 py-1 rounded-full border border-blue-100 inline-block mb-2">
                  {match.team}
                </span>
              )}
              <span className="text-slate-400 text-lg font-bold block mb-1">Heimspiel vs.</span>
              <span className="block">{match.opponent}</span>
            </>
          ) : (
            <>
              {match.event_categories?.name && (
                <span className="text-slate-600 text-xs font-bold bg-slate-100 px-3 py-1 rounded-full border border-slate-200 inline-block mb-2">
                  {match.event_categories.name}
                </span>
              )}
              <span className="block">{match.opponent}</span>
        ...
        <button
          onClick={() => {
            downloadICalendar([match], `dienstplan-${match.opponent.replace(/\s+/g, '-').toLowerCase()}-${match.id}.ics`);
          }}
