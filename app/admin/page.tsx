...
import EventCategoryManager, {
  buildEventCategorySlug,
} from '@/components/admin/EventCategoryManager';
...
export default function AdminPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [mRes, catRes, sRes, membersRes, slotsData] = await Promise.all([
        supabase
          .from('matches')
  ...
  const defaultSportCategoryId = useMemo(() => {
    const c =
      eventCategories.find((x) => x.slug === 'fussball') ?? eventCategories[0];
    return c?.id ?? 0;
  }, [eventCategories]);

  const openEditor = async (match?: Match) => {
    const defId = defaultSportCategoryId;
    if (match) {
      ...
      setEditForm({ 
        ...
        date: isoDate || '', 
        time: match.time, 
        location: match.location || 'Veranstaltungsort',
        team: match.team || '1. Mannschaft',
        event_category_id: match.event_category_id ?? defId,
      });
  ...
  const saveMatchMetadata = async () => {
    if (!editForm.opponent?.trim()) {
      setAlertDialog({
        isOpen: true,
        title: 'Fehler',
        message: 'Bezeichnung / Titel fehlt',
        variant: 'error',
      });
      return;
    }
    if (!editForm.event_category_id) {
      setAlertDialog({
        isOpen: true,
        title: 'Fehler',
        message: 'Bitte eine Event-Kategorie wählen (unter Einstellungen anlegen).',
        variant: 'error',
      });
      return;
    }
    ...
    const selectedCat = eventCategories.find(
      (c) => c.id === editForm.event_category_id
    );
    const sport = selectedCat?.ui_profile === 'sport';
    ...
    const payload = { 
        ...
        team: sport ? editForm.team : null,
        event_category_id: editForm.event_category_id,
    };
  ...
  const handleFormChange = (field: string, value: string) => {
    if (field === 'event_category_id') {
      setEditForm((prev) => ({ ...prev, event_category_id: Number(value) || 0 }));
      return;
    }
