...
import { EventCategory, ServiceType, Slot, Match } from '@/types';
import { getOpponentFieldLabel, isSportMatch } from '@/lib/match-display';
...
export default function MatchEditor({
  ...
  const syntheticMatch = {
    ...
    event_categories: selectedCategory ?? null,
  } as Match;
  const opponentLabel = getOpponentFieldLabel(syntheticMatch);
  const showTeamField = isSportMatch(syntheticMatch);
        ...
        <section className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select
                value={editForm.event_category_id ? String(editForm.event_category_id) : ''}
                onChange={(e) => onFormChange('event_category_id', e.target.value)}
              ...
              >
                {eventCategories.length === 0 && (
                  <option value="">Bitte unter Einstellungen anlegen</option>
                )}
            ...
            <div>
              ...
              <input 
                ...
                className="w-full p-3 bg-slate-50 rounded-xl text-sm font-black text-slate-900 outline-none border border-transparent focus:border-blue-100 focus:bg-blue-50/50 transition-all" 
                placeholder={showTeamField ? 'Gegner' : 'z. B. Dorf-Kirmes'} 
              />
            </div>
            ...
            <div className={showTeamField ? 'flex gap-3' : ''}>
              <div className={showTeamField ? 'flex-1' : ''}>
                <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-2 mb-1 block">
                  Ort
                </label>
                <input 
